...
export default class StageManager {
    ...
    setStageEnd(stageNumber, mapNumber, mapAttribute) {
        ...
        if (randomNumber <= 0.2) {
            console.log('작업 수행: 20% 확률로 실행되었습니다.');
        }
        ...
        this.scene.matterCollision.addOnCollideEnd({
            ...
            callback: eventData => {
                ...
                // 키보드 입력 이벤트 해제
                this.scene.input.keyboard.off('keydown-E', this.goToStore);
            }
        });
    ...
    goToStore() {
        this.scene.scene.launch('StoreScene', {
            ...
            mapNumber: this.scene.mapNumber,
            mapAttribute: 0,
            battleEnd: true,
    ...
    update() {
        // OverlayScene이 종료된 후 score를 확인하고 업데이트
        if (this.scene.returnStoreStatus !== undefined) {
            ...
            this.scene.heartIndicator.setHeart(this.scene.player.status.nowHeart, this.scene.player.status.maxHeart)
            TextIndicator.setText(this.scene.coinIndicatorText, `Coins: ${this.scene.player.status.coin}`);
